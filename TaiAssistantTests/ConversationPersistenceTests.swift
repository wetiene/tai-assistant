...
final class ConversationPersistenceTests: XCTestCase {
    ...
    func testPersistedGreetingStartIfNeededDoesNotReseed() throws {
        ...
        let vm2 = ConversationViewModel(store: store2, meal: meal, gym: ConversationTestSupport.makeGym(), strengthConversation: ConversationTestSupport.makeStrengthConversation(), liveTai: ConversationTestSupport.makeLiveTai(), gymPlanRepository: ConversationTestSupport.makeGymPlanRepository(), ownerID: "test.user", assistantName: "Tai")
        vm2.startIfNeeded()
